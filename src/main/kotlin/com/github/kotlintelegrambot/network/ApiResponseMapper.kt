...
internal class ApiResponseMapper {
    ...
    fun <T> mapToTelegramBotResult(apiResponse: Response<T>): TelegramBotResult<T>{
        if(!apiResponse.ok){
            return  TelegramBotResult.Error.HttpError(
                httpCode = apiResponse.errorCode as Int, description = apiResponse.errorDescription
            )
        }
        return TelegramBotResult.Success(apiResponse.result as T)
    }
